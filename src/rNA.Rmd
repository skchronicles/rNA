...
multiQC = transform(merge(multiQC, as.data.frame(pca$x[,1:3]), by='row.names', all=TRUE), row.names=Row.names, Row.names=NULL)
  ...
  DT::datatable(
    ...
    options = list(
      ...
      columnDefs = list(
        list(
          visible = FALSE,
          targets = c(1, 11, 14, 16, 18, 19, 21:23, 27, 29, 30, 31)   # hide columes
        )
      ), 
      buttons = list(
