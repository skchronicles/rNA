...
g <- ggplot(multiQC, aes(PC1, PC2, color = TSS_Site), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "Tissue Source", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC2 (",pc2,"%)"))
...
g <- ggplot(multiQC, aes(PC1, PC2, color = inner_distance_maxima), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "Inner Distance", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC2 (",pc2,"%)")) + 
          scale_colour_gradientn(colours = viridis::viridis(100))
...
Corr plots {data-orientation=columns data-icon="ion-grid"}
=====================================
...
cheatmap <- ComplexHeatmap::Heatmap(as.matrix(t(pca_exp)), 
                    col=colorRamp2(seq(0, max(pca_exp, na.rm = T), length.out = 20), colorRampPalette(brewer.pal(9, "YlGnBu"))(20)),
