...
#tincounts = read.table(file = 'data/TCGA-GBM_TINs.txt', sep = '\t', header = TRUE, row.names = 1)
...
multiQC = read.table(file = 'data/multiqc_matrix.txt', sep = '\t', header = TRUE)
...
# filter_slider(
#   id = "avg_sequence_length",
#   label = "Average Sequence Length",
#   sharedData = shared_metadata,
#   column = ~avg_sequence_length,
#   step = 5,
#   sep = "",
#   ticks = FALSE
# )
...
# # % UniVec
# filter_slider(
#   id = "uni_vec_percent_aligned",
#   label = "% UniVec",
#   sharedData = shared_metadata,
#   column = ~uni_vec_percent_aligned,
#   sep = "",
#   ticks = FALSE
# )
  ...
  DT::datatable(
    ...
    options = list(
      ...
      columnDefs = list(
        list(
          visible = FALSE,
          targets = c(1, 11, 14, 16, 18, 19, 21:23, 27, 31, 32, 33, 34, 35, 36)   # hide columes
        )
      ), 
      buttons = list(
...
p <- plot_ly(shared_metadata, x = ~PC1, y = ~PC2, z = ~PC3, color=cgroups, colors=cpalette, hoverinfo="text",
            text = ~paste('</br><b> Sample: ', Sample, '</br> Tissue Source: ', TSS_Site, '</b></br></br> % Aligned: ', percent_aligned, 
                     ...
                     '</br> % Intronic: ', pct_intronic_bases, '</br></br> TIN: ', median_tin, '</br> Sequence Range: ', sequence_length,
                     '</br> Inner Distance: ', inner_distance_maxima, '</br> Insert Size: ', median_insert_size) ) %>% 
...
p <- plot_ly(shared_metadata, x = ~PC1_tin, y = ~PC2_tin, z = ~PC3_tin, color=cgroups, colors=cpalette, hoverinfo="text",
            text = ~paste('</br><b> Sample: ', Sample, '</br> Tissue Source: ', TSS_Site, '</b></br></br> % Aligned: ', percent_aligned, 
                     '</br> % Dup: ', percent_duplication, '</br> % Coding: ', pct_coding_bases,
                     '</br> % Intronic: ', pct_intronic_bases, '</br></br> TIN: ', median_tin, '</br> Sequence Range: ', sequence_length,
                     '</br> Inner Distance: ', inner_distance_maxima, '</br> Insert Size: ', median_insert_size) ) %>% 
...
```{r colored-by-tss-site}
# Gene Expression PCA colored by TSS Site 
g <- ggplot(multiQC, aes(PC1, PC2, color = TSS_Site), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "Tissue Source", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC1 (",pc2,"%)"))
...
g <- ggplot(multiQC, aes(PC1, PC2, color = median_tin), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "TIN", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC1 (",pc2,"%)"))
...
g <- ggplot(multiQC, aes(PC1, PC2, color = percent_duplication), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "% Dups", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC1 (",pc2,"%)"))
...
g <- ggplot(multiQC, aes(PC1, PC2, color = percent_aligned), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "% Aligned", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC1 (",pc2,"%)"))
...
g <- ggplot(multiQC, aes(PC1, PC2, color = median_cv_coverage), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "CV Coverage", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC1 (",pc2,"%)"))
...
g <- ggplot(multiQC, aes(PC1, PC2, color = median_3prime_bias), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "3' Prime Coverage", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC1 (",pc2,"%)"))
...
g <- ggplot(multiQC, aes(PC1, PC2, color = median_insert_size), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "Insert Size", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC1 (",pc2,"%)"))
...
g <- ggplot(multiQC, aes(PC1, PC2, color = inner_distance_maxima), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "Inner Distance", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC1 (",pc2,"%)"))
...
g <- ggplot(multiQC, aes(PC1, PC2, color = pct_coding_bases), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "% Coding", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC1 (",pc2,"%)"))
...
g <- ggplot(multiQC, aes(PC1, PC2, color = rRNA_percent_aligned), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "% rRNA", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC1 (",pc2,"%)"))
...
g <- ggplot(multiQC, aes(PC1, PC2, color = percent_antisense_strand), xlab) + geom_point(size = multiQC$TissueType) + theme_minimal() +
        labs(color = "% Anti-sense", x = paste0("PC1 (",pc1,"%)"), y = paste0("PC1 (",pc2,"%)"))
...
Adapter sequences were removed using Cutadapt prior to mapping to hg38 reference genome. STAR was run in _two-pass_ mode where splice-junctions are collected and aggregated across all samples and provided to the second-pass of STAR. Gene expression levels were quantified using RSEM. The expected counts from RSEM are merged across samples to create a counts matrix for downstream analysis.
...
| RSeQC<sup>9</sup>          | 2.6.4   | **Quality-control step** to infer stranded-ness and read distributions over specific genomic features         |
